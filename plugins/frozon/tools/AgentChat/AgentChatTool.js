...
// Tool name as it appears in a mission config's `tools` list. ToolController_'s
// openTool/closeTool are keyed by this, not by the module name.
const AGENT_TOOL_NAME = 'AgentChat'
...
function openAgentChatFromLauncher(attempt = 0) {
    // The tool list is built asynchronously from the mission config, so the
    // launcher can be clicked before AgentChat is registered — retry briefly.
    if (!isAgentToolRegistered()) {
        if (attempt < 5) {
            setTimeout(() => openAgentChatFromLauncher(attempt + 1), 200)
        }
        return
    }
    ToolController_.openTool(AGENT_TOOL_NAME)
}

function isAgentToolRegistered() {
    return !!(
        ToolController_ &&
        Array.isArray(ToolController_.tools) &&
        ToolController_.tools.some((t) => t && t.name === AGENT_TOOL_NAME)
    )
}
...
const AgentChatTool = {
    height: 0,
    width: 'full',
    // Separated tools are auto-opened by ToolController_ only while `made` is
    // false, and openTool/closeTool read it to stay idempotent.
    made: false,
    MMGISInterface: null,
    initialize() {
        hideToolbarButton()
    ...
    function wireHeaderControls(panel) {
        panel
            ...
            ?.addEventListener('click', () => {
                ...
                cleanup()
                try {
                    window.ToolController_?.closeTool?.(AGENT_TOOL_NAME)
                } catch (_) {}
                setTimeout(() => {
